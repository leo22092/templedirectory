...
### `temples.submitted_by`
Stores the name entered by the community member when submitting or correcting a temple.
This field is included in the public JSON export (`data/*.json`) **only** when the
temple's `admin_label` is `COMMUNITY SUBMITTED` or `COMMUNITY CORRECTED`, so that
bulk-imported records (where `submitted_by` is `'admin'` or null) never show a name
on the public site. The filter lives in `COMMUNITY_LABELS` inside
...
### Automated D1 Export & All-India Search Index Workflow (GitHub Actions)

The repository includes an automated workflow at `.github/workflows/export-d1-json.yml` that syncs approved D1 changes back to static files and Cloudflare Pages automatically.

```
D1 Database (temples)
       │ (Daily cron @ 00:00 IST or manual dispatch)
       ▼
GitHub Action (.github/workflows/export-d1-json.yml)
       │ 1. node scripts/export-d1-to-json.mjs --write --yes --remote
       │ 2. node scripts/generate-index.mjs
       ▼
Commits updated data/*.json and data/index.json to main
       ▼
Cloudflare Pages auto-deploys via node scripts/build-static.mjs
```

#### Required Setup for Option 3 (Automated Action):
1. **Cloudflare Account ID & API Token**:
   - `CLOUDFLARE_ACCOUNT_ID`: Account ID from Cloudflare Dashboard (Workers & Pages sidebar).
   - `CLOUDFLARE_API_TOKEN`: Custom API Token with `Account -> D1 -> Edit/Read` permissions.
2. **GitHub Secrets**:
   - Store both in GitHub Repo → **Settings** → **Secrets and variables** → **Actions**.
3. **Workflow Permissions**:
   - GitHub Repo → **Settings** → **Actions** → **General** → set Workflow permissions to **Read and write permissions**.
4. **Index Generation Hook**:
   - Workflow step must run both `node scripts/export-d1-to-json.mjs --write --yes --remote` and `node scripts/generate-index.mjs` to keep `data/index.json` in sync.
...
| Topic | Status | Notes |
|---|---|---|
| All-India Live Submissions | Planned / Documented | Extend `/api/public-submissions` to support `state=all-india` and merge pending submissions into `loadStateData('all-india')` in `main.js`. |
| Automated D1→JSON publish CI | Implemented (Action configured) | `.github/workflows/export-d1-json.yml` exports D1 data and rebuilds `index.json` on daily cron and workflow dispatch. Requires repo secrets `CLOUDFLARE_ACCOUNT_ID` & `CLOUDFLARE_API_TOKEN`. |
| submittedBy in public JSON | Complete | `export-d1-to-json.mjs` now includes `submitted_by` as `submittedBy` for `COMMUNITY SUBMITTED` / `COMMUNITY CORRECTED` records. Card rendering and CSS were already in place. |
...
| SEO / SSG | Complete | Pre-rendered static pages in `dist/` generated by `scripts/build-static.mjs` |
...
| Dashboard state dropdown reloads D1 tabs | Fixed | `switchState()` now calls `loadDbTemples()` / `loadTempleRequests()` when those sections are currently active. |

---

_Last updated: 2026-08-12 by Antigravity_
