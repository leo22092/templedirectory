...
<h2>More Temples in ${esc(dn)}</h2><ul>${
    nearby.map(n=>{
...
// ── _redirects ────────────────────────────────────────────────────────────
const redirectLines=['# Old ?state= URLs → new path-based URLs'];
for(const sk of Object.keys(stateMap)) redirectLines.push(`/?state=${sk}  /temples/${sk}/  301`);
redirectLines.push('','# SPA fallback',`/*  /index.html  200`);
write(`${DIST}/_redirects`, redirectLines.join('\n'));
